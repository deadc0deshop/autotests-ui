...
class CreateCoursePage(BasePage):
    def __init__(self, page: Page):
        ...
        #Наименование заголовка страницы
        self.create_course_title = page.get_by_test_id('create-course-toolbar-title-text')
        self.create_course_button = page.get_by_test_id('create-course-toolbar-create-course-button')


        #Область пустого изображения
        self.preview_empty_view_icon = page.get_by_test_id('create-course-preview-empty-view-icon')
        self.preview_empty_view_title_text = page.get_by_test_id('create-course-preview-empty-view-title-text')
        self.preview_empty_view_description_text = page.get_by_test_id('create-course-preview-empty-view-description-text')
        ...
        self.preview_image_upload_title_text = page.get_by_test_id('create-course-preview-image-upload-widget-info-title-text')
        self.preview_image_upload_description_text = page.get_by_test_id('create-course-preview-image-upload-widget-info-description-text')
        ...
        self.exercises_empty_view_title_text = page.get_by_test_id('create-course-exercises-empty-view-title-text')
        self.exercises_empty_view_description_text = page.get_by_test_id('create-course-exercises-empty-view-description-text')
    ...
    def check_visible_image_preview_empty_view(self):
        ...
        expect(self.preview_empty_view_title_text).to_be_visible()
        expect(self.preview_empty_view_title_text).to_have_text('No image selected')

        expect(self.preview_empty_view_description_text).to_be_visible()
        expect(self.preview_empty_view_description_text).to_have_text('Preview of selected image will be displayed here')

    def check_visible_image_upload_view(self, is_image_upload: bool = False):
        expect(self.preview_image_upload_icon).to_be_visible()

        expect(self.preview_image_upload_title_text).to_be_visible()
        expect(self.preview_image_upload_title_text).to_have_text('Tap on "Upload image" button to select file')

        expect(self.preview_image_upload_description_text).to_be_visible()
        expect(self.preview_image_upload_description_text).to_have_text('Recommended file size 540X300')
    ...
    def check_visible_create_exercise_form(
        ...
        exercise_subtitle = self.page.get_by_test_id('create-course-exercise-{index}-box-toolbar-subtitle-text')
        exercise_title_input = self.page.get_by_test_id('create-course-exercise-form-title-{index}-input')
        exercise_description_input = self.page.get_by_test_id('create-course-exercise-form-description-{index}-input')

        expect(exercise_subtitle).to_be_visible()
        expect(exercise_subtitle).to_have_text(f"#{index +1} Exercise")

        expect(exercise_title_input).to_visible()
        expect(exercise_title_input).to_have_value(title)

        expect(exercise_description_input).to_visible()
    ...
    def fill_create_exercise_form(
        ...
        expect(exercise_description_input).fill(description)
        expect(exercise_description_input).to_have_value(description)
